...
export const useSuperAdminRoles = () => {
  ...
  const [superAdminRoleMappings, setSuperAdminRoleMappings] = useState<
    SuperAdminRoleMapping[]
  >([]);
  const [MasterAdminRoleMappings, setMasterAdminRoleMappings] = useState<
    MasterAdminRoleMapping[]
  >([]);
  const [groupedModuleActions, setGroupedModuleActions] = useState<
    ModuleWithActions[]
  >([]);
  ...
  const [masterAdminPaginationParams, setMasterAdminPaginationParams] =
  ...
  const [superAdminPaginationParams, setSuperAdminPaginationParams] =
    useState<PaginationParams>({
      page: 1,
      limit: 10,
      sort_by: "role_id",
  ...
  // Pagination metadata
  const [masterAdminPaginationMeta, setMasterAdminPaginationMeta] =
  ...
  const [superAdminPaginationMeta, setSuperAdminPaginationMeta] =
    useState<PaginationMeta>({
      page: 1,
      limit: 10,
      total_items: 0,
  ...
  const updateMasterAdminPaginationParams = useCallback(
  ...
  const updateSuperAdminPaginationParams = useCallback(
    (newParams: Partial<PaginationParams>) => {
  ...
  const fetchSuperAdminRoles = useCallback(
    async (params?: Partial<PaginationParams>) => {
      try {
        const queryParams = params
          ? { ...superAdminPaginationParams, ...params }
...
console.log("roles", data);
  ...
  const fetchSuperAdminRoleMappings = useCallback(
    async (params?: Partial<PaginationParams>) => {
      try {
        const queryParams = params
          ? { ...superAdminPaginationParams, ...params }
        ...
        const { data }: { data: SuperAdminRoleMappingsResponse } =
          await axios.get(`/api/v1/role/mappings?${queryString}`, {
            headers: getAuthHeaders(),
          });
...
console.log("fetchsuperadminmapping", data);
        ...
        if (data.meta) {
          ...
          setSuperAdminPaginationMeta({
            ...
            total_pages: Math.ceil(
              (data.total || data.data.length) / queryParams.limit
            ),
          });
        }
      } catch (error) {
  ...
  const fetchAssignedRoles = useCallback(
    async (params?: Partial<PaginationParams>) => {
      try {
        const queryParams = params
          ? { ...masterAdminPaginationParams, ...params }
          : masterAdminPaginationParams;

        const { data }: { data: AssignedUserRoleModuleActionsResponse } =
          await axios.get(
            `/api/v1/user/assigned?page=${queryParams.page}&limit=${queryParams.limit}&sort_by=assignment_date&order=asc`,
            {
              headers: getAuthHeaders(),
            }
          );
console.log("fetchassignroles", data);
        ...
        setMasterAdminRoleMappings(
          data.data.assignments.flatMap((assignment: AssignedModule) =>
        ...
        console.error(
          "Failed to fetch assigned user role module actions:",
          error
        );
  ...
  const getAssignedSubActionsForRoleModuleAction = useCallback(
    (roleId: number, moduleName: string, actionName: string) => {
  ...
  const getActionOptionsForModules = useCallback(
      ...
      superAdminRoleMappings.forEach((mapping) => {
        if (
          ...
          mapping.sub_actions.forEach((subAction) => {
            ...
            if (
              !seen.has(uniqueKey) &&
              !assignedSubActionIds.has(subAction.sub_action_id)
            ) {
              seen.add(uniqueKey);
              subActions.push({
  ...
  const handleCreateSuperAdminRoleMapping = useCallback(
    async (
      ...
      try {
        ...
        await axios.post("/api/v1/role/mappings", payload, {
          headers: {
            ...getAuthHeaders(),
        ...
        const errorMessage =
          ...
          "Unknown error";
        setMessage(
          `Failed to create super admin role mapping: ${errorMessage}`
        );
        return false;
      } finally {
        setIsLoading(false);
  ...
  const handleUpdateSuperAdminRoleMapping = useCallback(
    async (
      mappingId: number,
      superAdminRoleId: number,
      selectedModules: string[],
      ...
      try {
        ...
        await axios.put(
          `/api/v1/role/mappings?super_admin_role_mappings_id=${mappingId}`,
          payload,
          {
            headers: {
              ...getAuthHeaders(),
        ...
        const errorMessage =
          ...
          "Unknown error";
        setMessage(
          `Failed to update super admin role mapping: ${errorMessage}`
        );
        return false;
      } finally {
        setIsLoading(false);
  ...
  const handleCreateRole = useCallback(
    async (roleData: {
      ...
      try {
        ...
        // Throw the error with the API's error message
        const errorMessage =
          error?.response?.data?.detail ||
          error?.response?.data?.message ||
          error.message ||
