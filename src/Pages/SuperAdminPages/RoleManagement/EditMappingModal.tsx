...
const EditMappingModal: React.FC<EditMappingModalProps> = ({
  ...
  const [existingData, setExistingData] = useState<{
    modules: string[];
    actions: { id: string; action_id: number }[];
    subActions: { name: string; parentAction: string }[];
  }>({ modules: [], actions: [], subActions: [] });
  const [newSelections, setNewSelections] = useState<{
    modules: string[];
  ...
  useEffect(() => {
    if (mapping && isOpen) {
      ...
      // Extract actions from the grouped mapping with both name and ID
      const actions = mapping.actions?.map((action: any) => ({
        id: action.action_name,
        action_id: action.action_id || getActionOptionsForModules([mapping.module_name]).find(a => a.id === action.action_name)?.action_id || 0
      })) || [];
     
      // Extract sub-actions with their parent action information
      const subActions = mapping.actions?.flatMap((action: any) =>
        action.sub_actions?.map((sub: any) => ({
          name: sub.sub_action_name,
          parentAction: action.action_name
  ...
  const actionOptions = useMemo(() => {
    ...
    const allActions = getActionOptionsForModules(allModules);
    // Filter out actions that are already in existing data
    return allActions.filter(action => !existingData.actions.some(existing => existing.id === action.id));
  }, [existingData.modules, newSelections.modules, existingData.actions, getActionOptionsForModules]);
  ...
  const subActionOptions = useMemo(() => {
    ...
    const allActionNames = [
      ...existingData.actions.map(a => a.id),
      ...newSelections.actions.map(a => a.id)
    ];
   
    if (allModules.length === 0 || allActionNames.length === 0) return [];


    const allSubActions: any[] = [];
   
    // Get sub-actions for each action
    allActionNames.forEach((action) => {
      const subs = getSubActionOptionsForModulesAndActions(
        allModules,
        action,
        selectedRole || undefined
  ...
  const removeExistingItem = (
    type: 'modules' | 'actions' | 'subActions',
    item: string | { name: string; parentAction: string } | { id: string; action_id: number }
  ) => {
    if (type === 'subActions' && typeof item === 'object' && 'name' in item) {
      setExistingData(prev => ({
        ...prev,
        subActions: prev.subActions.filter(
          s => !(s.name === item.name && s.parentAction === item.parentAction)
    ...
    } else if (typeof item === 'string' || (typeof item === 'object' && 'id' in item)) {
      setExistingData(prev => {
        const updated = {
          ...prev,
          [type]: prev[type].filter((i: any) =>
            type === 'actions' && typeof item === 'object' && 'id' in item
              ? i.id !== item.id
              : i !== item
          ),
        };
        ...
        // If removing an action, remove all its related sub-actions
        if (type === 'actions') {
          const actionName = typeof item === 'object' && 'id' in item ? item.id : item;
          // Remove related sub-actions from existingData
          updated.subActions = prev.subActions.filter(s => s.parentAction !== actionName);


          // Remove related sub-actions from newSelections
          setNewSelections(prevSel => ({
            ...prevSel,
            subActions: prevSel.subActions.filter(s => s.parentAction !== actionName),
            // Also remove the action from newSelections if it exists there
            actions: prevSel.actions.filter(a => a.id !== actionName)
          }));
        }
      ...
      // If removing a module, remove all related actions and sub-actions
      if (type === 'modules') {
        // Clear all actions and sub-actions when module is removed
        setExistingData(prev => ({
          ...prev,
          actions: [],
          subActions: []
  ...
  const addNewSelection = (type: 'modules' | 'actions', item: string | { id: string; action_id: number }) => {
    setNewSelections(prev => ({
      ...prev,
      [type]: type === 'actions' && typeof item === 'object'
        ? prev[type].some((a: any) => a.id === item.id)
          ? prev[type].filter((a: any) => a.id !== item.id)
          : [...prev[type], item]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const allModules = [...existingData.modules, ...newSelections.modules];
    const allActions = [...existingData.actions, ...newSelections.actions];
    const allSubActions = [...existingData.subActions, ...newSelections.subActions];


    if (!selectedRole || allModules.length === 0 || allActions.length === 0) {
      setError("Please select role, at least one module and one action");
      return;
    }


    // Check if each action has at least one sub-action
    const actionsWithoutSubActions = allActions.filter(action =>
      !allSubActions.some(s => s.parentAction === action.id)
    );


    if (actionsWithoutSubActions.length > 0) {
      setError(`The following actions need at least one sub-action: ${actionsWithoutSubActions.map(a => a.id).join(', ')}`);
      return;
    }
    ...
    try {
      const results = await Promise.all(
        allActions.map(async (action) => {
          const subActionsForAction = allSubActions
            .filter((s) => s.parentAction === action.id)
            .map((s) => s.name);


          if (subActionsForAction.length === 0) {
            return true; // Skip actions without sub-actions
          }
            ...
            <div className="space-y-4">
              ...
              {existingData.actions.length > 0 && (
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {existingData.actions.map((action) => (
                      <div key={action.id} className="flex items-center justify-between bg-purple-50 p-3 rounded-lg border border-purple-100">
                        <span className="text-sm text-purple-900 font-medium">{action.id}</span>
                        <button
                          type="button"
                          onClick={() => removeExistingItem('actions', action)}
                          className="text-red-600 hover:text-red-800 p-1 hover:bg-red-100 rounded transition-colors"
                      ...
                      <div className="p-2 space-y-1">
                        {actionOptions.map((action) => (
                          <label key={action.id} className="flex items-center p-2 hover:bg-gray-50 rounded cursor-pointer">
                            <input
                              type="checkbox"
                              checked={newSelections.actions.some(a => a.id === action.id)}
                              onChange={() => addNewSelection('actions', { id: action.id, action_id: action.action_id })}
                              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-3"
                            />
                            <span className="text-sm text-gray-700">{action.label}</span>
                          </label>
