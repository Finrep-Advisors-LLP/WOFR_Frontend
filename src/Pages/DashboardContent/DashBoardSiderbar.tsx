...
  // Updated isChildRouteActive to correctly handle deeply nested paths
  const isChildRouteActive = (children: any[], currentBasePath: string): boolean => {
    return children?.some((child) => {
      const fullPath = child.path ? `${currentBasePath}/${child.path}`.replace(/\/\//g, '/') : currentBasePath;
      if (child.children) {
...
      return location.pathname === `/dashboard${fullPath}` || location.pathname.startsWith(`/dashboard${fullPath}/`);
    }) ?? false;
  };

  const renderNestedChildren = (children: any[], parentName: string, level: number = 1, currentBasePath: string) => {
    return children.map((child) => {
      // Construct the newBasePath for the current child
      const newBasePath = child.path ? `${currentBasePath}/${child.path}`.replace(/\/\//g, '/') : currentBasePath;
...
                  <span className={`flex-shrink-0 ${isChildRouteActive(child.children, newBasePath) ? "text-blue-600" : "text-gray-700 group-hover:text-black"}`}>
                    {child.icon}
                  </span>
                  {isOpen && <span className="ml-3 truncate font-semibold">{child.name}</span>}
                </div>
                {isOpen && (
                  <span className={`flex-shrink-0 transition-transform duration-200 ${expandedItems[childKey] ? 'rotate-180' : ''}`}>
                    <ChevronDown size={16} />
                  </span>
                )}
              </button>
...
                  {renderNestedChildren(child.children, childKey, level + 1, newBasePath)}
                </div>
              )}
            </>
          ) : (
            <NavLink
              to={`/dashboard${newBasePath}`} // This is the corrected line
              className={({ isActive }) =>
...
  (route.path && (location.pathname === `/dashboard/${route.path}` || location.pathname.startsWith(`/dashboard/${route.path}/`))) ||
  ((visibleChildren ?? []).length > 0 && isChildRouteActive(visibleChildren ?? [], route.path || ''));
...
                    <div className={`${isOpen ? "mt-1 space-y-1" : "space-y-1"}`}>
                     {renderNestedChildren(item.visibleChildren, item.name, 1, item.path || '')}
                    </div>
                  )}
                </>
              ) : (
