...
import React, { useState, useRef, useEffect } from "react";
import { ChevronDown, X, AlertTriangle, Loader2 } from "lucide-react";
import { LeaseFormData } from "../../../../types";
import useMaster from "../../../../hooks/useMaster";
...
export const LessorDetails: React.FC<LeaseBasicInfoProps> = ({
  ...
  const [lessorsLoading, setLessorsLoading] = useState(true);
  const [departmentsLoading, setDepartmentsLoading] = useState(true);

  const isMultiEntityMode = formData.hasMultiEntityAllocation;
  const isLessorSplitMode = formData.hasLessorAllocation;

  // Load master data with loading states
  useEffect(() => {
  ...
  useEffect(() => {
    const loadLessors = async () => {
      try {
        setLessorsLoading(true);
        await fetchLessors();
      } finally {
        setLessorsLoading(false);
      }
    };
    loadLessors();
  }, []);
  ...
  // Get departments for specific entity based on actual entity-department relationship
  const getDepartmentsForEntity = (entityId: string) => {
    ...
    return entity.departments.map(dept => ({
      label: dept.department_name,
  ...
  // Check if master data is available and loaded
  const hasRequiredMasterData = !entitiesLoading && !lessorsLoading && entities.length > 0 && LessorData.length > 0;
  const isAnyDataLoading = entitiesLoading || lessorsLoading || departmentsLoading;
  const showMasterDataWarning = !isAnyDataLoading && (entities.length === 0 || LessorData.length === 0);
      ...
      {isAnyDataLoading && (
        <div className="mb-6 p-4 bg-blue-50 border-l-4 border-blue-500 rounded-md">
          <div className="flex items-start">
            ...
            <div>
              <h3 className="text-blue-800 font-medium">Loading Master Data...</h3>
              <p className="text-blue-700 mt-1">Please wait while we load the required data.</p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-red-700 mt-2 text-sm">
                Navigate to the respective master sections and create the required data.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-6">
        ...
        <div>
          ...
          <MultiSelectDropdown
            ...
            singleSelect={!isMultiEntityMode}
            disabled={disabled || !hasRequiredMasterData || isAnyDataLoading}
          />
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div>
          ...
          <MultiSelectDropdown
            ...
            onSelectionChange={handleLessorChange}
            placeholder={
              lessorsLoading ? "Loading lessors..." :
              isLessorSplitMode ? "Select Lessor(s)" : "Select a Lessor"
            }
            singleSelect={!isLessorSplitMode}
            disabled={disabled || !hasRequiredMasterData || isAnyDataLoading}
      ...
      {!readOnly && (
        <div className="mt-8 flex justify-between">
          ...
          <button
            type="button"
            onClick={onNext}
            disabled={!canProceed || !hasRequiredMasterData || isAnyDataLoading}
            className={`px-4 py-2 rounded-md transition-colors ${
              !canProceed || !hasRequiredMasterData || isAnyDataLoading
                ? "bg-gray-400 text-gray-200 cursor-not-allowed"
                : "bg-[#008F98] text-white hover:bg-[#007A82] cursor-pointer"
            }`}
          >
