...
const CreateLease: React.FC = () => {
  ...
  const { filteredSteps, displayCurrentStep } = useMemo(() => {
    const allSteps = [
      ...
      },
      {
        ...
        name: "Lease Payment Details",
        status: currentStep === 3 ? "current" : currentStep > 3 ? "complete" : "upcoming",
        skip: formData.hasCashflow, // Skip this step when cashflow is enabled
      },
      {
        id: 4,
        name: "Security Deposit",
    ...
    // Reassign IDs to filtered steps
    const updatedFilteredSteps = filtered.map((step, index) => ({
      ...step,
      id: index + 1,
      status:
  ...
  const handleNext = async () => {
    ...
    // Handle step navigation based on cashflow mode
    if (formData.hasCashflow) {
      // Skip step 3 (Lease Payment Details) when cashflow is enabled
      if (currentStep === 2) {
        setCurrentStep(4); // Go to Security Deposit
      } else if (currentStep < 6) {
        setCurrentStep(currentStep + 1);
      }
    } else {
  ...
  const handlePrevious = async () => {
    ...
    if (formData.hasCashflow) {
      // Skip step 3 (Lease Payment Details) when cashflow is enabled
      if (currentStep === 4) {
        setCurrentStep(2); // Go back to Lessor & Lessee Details
      } else if (currentStep > 1) {
        setCurrentStep(currentStep - 1);
      }
    } else {
